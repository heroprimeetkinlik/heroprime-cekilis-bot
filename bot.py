...
# AYARLAR
# =========================================================
# Railway > Variables bölümünde BOT_TOKEN tanımla.
# Tokenı kaynak koda yazma.
BOT_TOKEN = '8855111211:AAGwRspv-AFsAakM_MhVVOxbL17DjKei0Q8'
...
ADMIN_RESULT_CHAT_ID = int(os.getenv("ADMIN_RESULT_CHAT_ID", "8845737995"))
...
def acquire_single_instance_lock() -> None:
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"Bot lock dosyası mevcut: {BOT_LOCK_FILE}. "
                "Aynı container içinde başka bir bot instance'ı çalışıyor olabilir."
            )
    ...
    try:
        ...
        try:
            ...
            _bot_lock_handle = None
        raise RuntimeError(
            "Bu container içinde HEROPRIME botunun başka bir instance'ı zaten çalışıyor."
        )
...
def release_single_instance_lock() -> None:
    ...
    # Windows fallback için oluşturulan dosyayı temizle.
    try:
...
# =========================================================
# ADMİNLER / YARDIMCI FONKSİYONLAR
# =========================================================
def get_admin_ids():
    admin_ids = set()
...
def is_allowed_chat(update: Update) -> bool:
    ...
    return get_chat_username(chat) in {
        normalize_username(name) for name in ALLOWED_CHAT_USERNAMES
    }
...
async def post_init(application: Application):
    try:
        ...
        if webhook_url:
            logger.warning("Aktif Telegram webhook bulundu: %s", webhook_url)
            await application.bot.delete_webhook(drop_pending_updates=False)
            logger.info("Telegram webhook başarıyla kaldırıldı.")
        else:
            logger.info("Aktif Telegram webhook bulunamadı.")

        logger.info("Polling modu hazırlanıyor.")
    except Exception as error:
        logger.exception("Webhook kontrolü/silinmesi sırasında hata: %s", error)
        raise
...
# =========================================================
async def can_manage_giveaway(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if user and chat and chat.type in ("group", "supergroup"):
        try:
            ...
            logger.warning("Grup admin kontrolü başarısız: %s", error)
...
# =========================================================
async def is_test_channel_member(context: ContextTypes.DEFAULT_TYPE, user_id: int) -> bool:
    try:
        ...
        logger.warning("Test kanalı üyelik kontrolü başarısız: %s", error)
        return False
...
# ÇEKİLİŞ METİNLERİ
# =========================================================
def build_giveaway_text(custom_text: str, winner_count: int, participant_count: int) -> str:
    return (
        f"{custom_text}\n\n"
...
# ADMIN DM
# =========================================================
async def send_admin_dm(context: ContextTypes.DEFAULT_TYPE, text: str):
    try:
...
# /CEKILISMET
# =========================================================
async def giveaway_text_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    message = update.effective_message
    user = update.effective_user
    if not user or not message:
        return
    ...
    if message.chat.type != "private":
        await message.reply_text(
            "🔒 /cekilismet komutu yalnızca botla özel mesajda kullanılabilir."
        )
        return
...
# ADMIN YENİ METİN
# =========================================================
async def save_new_giveaway_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    message = update.effective_message
    user = update.effective_user
    if not user or not message:
        return

    if not is_admin(user.id) or message.chat.type != "private":
        return

    if not context.user_data.get("waiting_for_giveaway_text", False):
        return
    ...
    if len(new_text) > 3500:
        await message.reply_text("❌ Çekiliş metni çok uzun. Lütfen 3500 karakterden kısa bir metin gönder.")
        return

    # Kullanıcının girdiği metni HTML'den bağımsız güvenli şekilde sakla.
    save_giveaway_text(escape(new_text))
    context.user_data["waiting_for_giveaway_text"] = False

    await message.reply_text(
        "✅ <b>Çekiliş metni güncellendi.</b>\n\n"
        f"{escape(new_text)}\n\n"
        "Bu metin bundan sonraki /cekilis X komutlarında otomatik kullanılacak.",
        parse_mode="HTML",
    )
...
# /İPTAL
# =========================================================
async def cancel_text_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    message = update.effective_message
    user = update.effective_user
    if not user or not message:
        return
    ...
    context.user_data["waiting_for_giveaway_text"] = False
    await message.reply_text("❌ Çekiliş metni değiştirme işlemi iptal edildi.")
...
async def start_giveaway(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if winner_count < 1:
        await message.reply_text("❌ Kazanan sayısı en az 1 olmalı.")
        return
    ...
    initial_text = build_giveaway_text(get_giveaway_text(), winner_count, 0)
    ...
    try:
        giveaway_message = await context.bot.send_message(
            ...
            text=initial_text,
            parse_mode="HTML",
            # Geçici olarak 0 kullanılır; DB kaydından sonra gerçek ID ile güncellenir.
            reply_markup=build_join_keyboard(0),
        )
    except Exception as error:
        logger.exception("Çekiliş mesajı gönderilemedi: %s", error)
        try:
            await message.reply_text(
                "❌ Çekiliş mesajı gönderilemedi.\n\n"
                "Botun bu kanal/grupta mesaj gönderme yetkisini kontrol et."
            )
        except Exception:
            pass
        return

    started_by = update.effective_user.id if update.effective_user else next(
        iter(ADMIN_IDS), ADMIN_RESULT_CHAT_ID
    )
    ...
    cursor.execute(
        ...
        VALUES (?, ?, ?, ?, 1, ?)
        """,
        (chat.id, giveaway_message.message_id, started_by, winner_count, utc_now()),
    )
    ...
    try:
        ...
        logger.warning("KATIL butonu ID ile güncellenemedi: %s", error)

    chat_username = get_chat_username(chat)
    chat_link = f"@{chat_username}" if chat_username else str(chat.id)
    chat_title = chat.title or chat_username or str(chat.id)

    await send_admin_dm(
        context,
        "🚀 <b>ÇEKİLİŞ BAŞLADI</b>\n\n"
        f"💬 Sohbet: <b>{escape(chat_title)}</b>\n"
        f"🔗 {escape(chat_link)}\n"
        f"🏆 Kazanan sayısı: <b>{winner_count}</b>\n"
        "👥 Katılımcı: <b>0</b>\n\n"
...
async def join_giveaway_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    giveaway_chat_id = giveaway["chat_id"]

    try:
        target_chat = await context.bot.get_chat(giveaway_chat_id)
        target_username = get_chat_username(target_chat)

        if target_chat.type not in ("group", "supergroup", "channel"):
            raise ValueError("Desteklenmeyen sohbet türü")
        ...
        if target_username not in {
            ...
            connection.close()
            await query.answer(
                "❌ Bu çekiliş bu sohbette kullanılamaz.",
                show_alert=True,
            )
            return
    except Exception as error:
        connection.close()
        logger.warning("Çekiliş sohbeti doğrulanamadı: %s", error)
        await query.answer("❌ Çekiliş sohbeti doğrulanamadı.", show_alert=True)
        return
    ...
    entered_username = telegram_username or telegram_name

    try:
        connection.execute(
            """
            INSERT INTO participants
            (giveaway_id, telegram_user_id, telegram_username, telegram_name,
             entered_username, joined_at)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (
                giveaway_id,
    ...
    new_text = build_giveaway_text(
        get_giveaway_text(), giveaway["winner_count"], participant_count
    )
    ...
    try:
        ...
        logger.warning("Katılım sonrası çekiliş mesajı güncellenemedi: %s", error)
...
async def stop_giveaway(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if active["chat_id"] != chat.id:
        await message.reply_text("❌ Bu sohbette aktif çekiliş bulunmuyor.")
        return
    ...
    connection.execute(
        "UPDATE giveaways SET active = 0, ended_at = ? WHERE id = ?",
        (utc_now(), active["id"]),
    )
    connection.commit()
    connection.close()
    ...
    if not participant_list:
        ...
        try:
            ...
            logger.warning("Çekiliş bitiş mesajı güncellenemedi: %s", error)
    ...
    winner_count = min(active["winner_count"], participant_count)
    winners = secrets.SystemRandom().sample(participant_list, winner_count)
    ...
    for index, winner in enumerate(winners, start=1):
        ...
        identity = f"@{telegram_username}" if telegram_username else winner["telegram_name"]
        identity_html = escape(identity)
        group_lines.append(f"{index}. {identity_html}")
        admin_lines.append(
    ...
    try:
        ...
        logger.warning("Sonuç mesajı güncellenemedi: %s", error)
    ...
    try:
        ...
        logger.warning("Grup sonuç özeti gönderilemedi: %s", error)

    logger.info(
        "Çekiliş tamamlandı: giveaway_id=%s participants=%s winners=%s",
        active["id"], participant_count, winner_count,
    )
...
async def giveaway_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    active = get_active_giveaway()
    if not active:
        await message.reply_text("ℹ️ Şu anda aktif bir çekiliş yok.")
        return
...
def main():
    ...
    if not ADMIN_IDS:
        raise RuntimeError("Admin ID bulunamadı.")
    ...
    logger.info("HEROPRIME Çekiliş Botu başlatılıyor...")
    logger.info(
        "İzin verilen sohbetler: %s",
        ", ".join(f"@{name}" for name in sorted(ALLOWED_CHAT_USERNAMES)),
    )
    logger.info("Admin sonuç hesabı: %s", ADMIN_RESULT_USERNAME)
    logger.info("Polling modu aktif.")
    ...
    try:
        ...
        application.run_polling(
            ...
            # Railway yeniden deploy edildiğinde eski pending update'lerin
            # topluca işlenmesini engeller.
        ...
        if (
            ...
            or "terminated by other getUpdates request" in error_text
        ):
            logger.error(
                "Telegram 409 Conflict: Bu BOT_TOKEN ile başka bir polling "
                "instance'ı getUpdates kullanıyor."
            )
        else:
            raise
